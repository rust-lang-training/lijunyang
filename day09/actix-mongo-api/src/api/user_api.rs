...
pub async fn create_user(db: Data<MongoRepo>, new_user: Json<User>) -> HttpResponse {
    ...
    let use_detail = db.create_user(data).await;
    match use_detail {
...
pub async fn update_user(
    ...
    let data = User {
        ...
        location: new_user.location.to_string(),
        title: new_user.title.to_string(),
    };
    let update_detail = db.update_user(&id, data).await;
    match update_detail {
        Ok(update) => {
            if update.matched_count == 1 {
                ...
                return HttpResponse::NotFound().body("No use found with specified ID");
            }
        }
        Err(err) => HttpResponse::InternalServerError().body(err.to_string()),
    }
    ...
    match result {
        Ok(res) => {
            if res.deleted_count == 1 {
                return HttpResponse::Ok().json("User with specified ID deleted successfully");
            }
